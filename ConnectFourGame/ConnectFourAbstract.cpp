...
int ConnectFourAbstract::playGame(){
    ...
    if(isBoardInitialized== false){
            ...
            cout<<"Enter dimension Y"<<endl;

            getline(cin,DimeIn);

            DimensionY=atoi(DimeIn.c_str());

            cout<<"Enter dimension X"<<endl;
                ...
                if (GetPosToVector(TempPos) == type) {
                    for (int i = 0; i < 4; ++i) {
                        if (GetPosToVector(TempPos + (DimensionX + 1) * i)!= type)
                            break;
                        if (i == 3) {
                            SetPosToVector(TempPos + (DimensionX + 1) * 0  ,type + ('a' - 'A')) ;
...
int ConnectFourAbstract::CheckWin(const int &position,const char &type){
